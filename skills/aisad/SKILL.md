---
name: aisad
description: Report local Claude Code and Codex usage as text or JSON, analyze session costs and tokens, build a dashboard, and update AISAD. Use for usage summaries, model/provider breakdowns, weekly comparisons, cache analysis, and AISAD installation or updates.
---
...
Both `usage` and `run` check for a newer stable GitHub release at most once every 24 hours when invoked, apply an available update, and then run the collector. Updates replace this skill's instructions, helper and collector together. If code was updated, reread this SKILL.md before interpreting the results. A failed check keeps the installed version usable. Use `usage --offline` or `run --offline` when the user wants no network activity; a first installation needs a bundled runtime or a successful update.
